...
(SELECT BI.POST_CODE "POST_CODE", BI.USER_CODE "USER_CODE", BI.WRITE_DATE "WRITE_DATE"
     , BI.HITCOUNT "HITCOUNT", BI.TITLE "TITLE"
     , (SELECT COUNT(*)
        FROM TBL_REC_INFORM
...
(SELECT BV.POST_CODE "POST_CODE", BV.USER_CODE "USER_CODE", BV.WRITE_DATE "WRITE_DATE"
     , BV.HITCOUNT "HITCOUNT", BV.TITLE "TITLE"
     , (SELECT COUNT(*)
...
(SELECT BS.POST_CODE "POST_CODE", BS.USER_CODE "USER_CODE", BS.WRITE_DATE "WRITE_DATE"
     , BS.HITCOUNT "HITCOUNT", BS.TITLE "TITLE"
     , (SELECT COUNT(*)
...
(SELECT BQ.POST_CODE "QPOST_CODE", BQ.USER_CODE "QUSER_CODE", BQ.WRITE_DATE "QWRITE_DATE"
     ...
     , BA.POST_CODE "APOST_CODE", BA.USER_CODE "AUSER_CODE", BA.WRITE_DATE "AWRITE_DATE"
...
FROM TBL_BOARD_STUDYREVIEW BR JOIN TBL_INTEREST_MC IM
...
AS
SELECT SO.STUDY_CODE "STUDY_CODE", SO.USER_CODE "USER_CODE", SO.STUDY_DESC "STUDY_DESC"
     , (SELECT STUDY_TYPE
        FROM TBL_STUDY_TYPE
        WHERE STUDY_TYPE_CODE = SO.STUDY_TYPE_CODE) "STUDY_TYPE"
     , SO.START_DATE "START_DATE", SO.END_DATE "END_DATE"
     , (SELECT MEMNUM
        FROM TBL_MEMNUM
        WHERE MEMNUM_CODE = SO.MIN_MEM_CODE) "MIN_MEM"
     , (SELECT MEMNUM
        FROM TBL_MEMNUM
        WHERE MEMNUM_CODE = SO.MAX_MEM_CODE) "MAX_MEM"
     , SO.STUDY_NAME "STUDY_NAME"
     , (SELECT RANK 
        FROM TBL_USER_RANK
        WHERE RANK_CODE = SO.MIN_RANK) "MIN_RANK"
     , SO.CLOSE_DATE "CLOSE_DATE"
     , (SELECT INTEREST_MC 
        FROM TBL_INTEREST_MC 
        WHERE INTEREST_MC_CODE = SO.INTEREST_MC_CODE) "INTEREST_MC"
     , (SELECT LOC_MC 
        FROM TBL_LOC_MC 
        WHERE LOC_MC_CODE = SO.LOC_MC_CODE) "LOC_MC"
...
AS
SELECT SM.STUDY_CODE "STUDY_CODE", WD.WEEKDAY "WEEKDAY", SM.START_TIME "START_TIME", SM.END_TIME "END_TIME"  
